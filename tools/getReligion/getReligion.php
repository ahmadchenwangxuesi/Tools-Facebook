...
error_reporting(0);
    ...
    $save_dir = "result/result_religion.txt";
    $save_dir_name = "result/result_name-religion.txt";
    ...
    sleep(3);
    $climate->br()->info('Starting retrieve religion your friends..');
    echo "\n";
    progress($progress);
    ...
    $colorstring = getName($n);
    echo $x.".". $colors->getColoredString(" $decode->name | $decode->religion", $warifp[$colorstring]) . "\n";

    $save = fopen($save_dir, 'a');
    $save_name = fopen($save_dir_name, 'a');

    fwrite($save, $decode->religion . "\n");
    fwrite($save_name, $decode->name . "|" . $decode->religion . "\n");
