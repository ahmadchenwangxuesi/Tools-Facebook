...
error_reporting(0);
    ...
    if($gender == "male"){
        ...
        $save = fopen('tmp/id_requests_friends_male.log', 'a');
        fwrite($save, $id ."\n");
        fclose($save);
	}
} 
    
    $list = "tmp/id_requests_friends_male.log" or die ("File ID not found!");
    $climate->br()->info('Retrieve gender male friends success');
    sleep(3);
    $climate->br()->info('Starting confirmation male friends..');
    echo "\n";
    progress($progress);
    ...
    if($wahyuarifpurnomo == true){
		echo $x.".". $colors->getColoredString(" ID : $id | Success accept ..", $warifp[$colorstring]) . "\n";
	} else {
        echo $x.".". $colors->getColoredString(" ID : $id | Failed to accept ..", $warifp[$colorstring]) . "\n";
    }}
     
    $climate->br()->shout('Done, confirmation your all friends based on gender male.');
    $climate->br()->info('Cleaning log..');
    echo "\n";
    progress($progress);
    $delete_id = "id_requests_friends.log";
    $delete_gender = "id_requests_friends_male.log";
