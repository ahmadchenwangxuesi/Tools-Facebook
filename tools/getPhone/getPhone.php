...
error_reporting(0);
    ...
    $save_dir = "result/result_phone.txt";
    $save_dir_name = "result/result_name-phone.txt";
    ...
    sleep(3);
    $climate->br()->info('Starting retrieve mobile number your friends..');
    echo "\n";
    progress($progress);
    ...
    $colorstring = getName($n);
    echo $x.".". $colors->getColoredString(" $decode->name | $decode->mobile_phone", $warifp[$colorstring]) . "\n";

    $save = fopen($save_dir, 'a');
    $save_name = fopen($save_dir_name, 'a');

    fwrite($save, $decode->mobile_phone . "\n");
    fwrite($save_name, $decode->name . "|" . $decode->mobile_phone . "\n");
