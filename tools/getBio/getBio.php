...
error_reporting(0);
    ...
    $save_dir = "result/result_bio.txt";
    $save_dir_name = "result/result_name-bio.txt";
    ...
    sleep(3);
    $climate->br()->info('Starting retrieve bio your friends..');
    echo "\n";
    progress($progress);
    ...
    $decode = json_decode($ahmadchenwangxuesi);
    $colorstring = getName($n);
    echo $x.".". $colors->getColoredString(" $decode->name | $decode->bio", $warifp[$colorstring]) . "\n";

    $save = fopen($save_dir, 'a');
    $save_name = fopen($save_dir_name, 'a');

    fwrite($save, $decode->bio . "\n");
    fwrite($save_name, $decode->name . "|" . $decode->bio . "\n");
