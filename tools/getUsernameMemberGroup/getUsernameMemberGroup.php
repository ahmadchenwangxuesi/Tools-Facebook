...
//error_reporting(0);
    ...
    $save_dir = "result/result_usernamemembergroup_" . $name_group .".txt";
    $save_dir_name = "result/result_name-usernamemembergroup_" . $name_group .".txt";
    ...
    sleep(3);
    $climate->br()->info('Starting retrieve username member on group ' . $name_group . '..');
    echo "\n";
    progress($progress);
    ...
    $colorstring = getName($n);
    echo $x.".". $colors->getColoredString(" $decode->name | $decode->username", $warifp[$colorstring]) . "\n";

    $save = fopen($save_dir, 'a');
    $save_name = fopen($save_dir_name, 'a');

    fwrite($save, $decode->username . "\n");
    fwrite($save_name, $decode->name . "|" . $decode->username . "\n");
